# All the steps performed below are on the UCI HAR Dataset 
...
setwd('/Users/Dipankar/Documents/UCI HAR Dataset/');
...
for (i in 1:length(colNames)) 
{
  ...
  colNames[i] = gsub("([Bb]odyaccjerkmag)","Body_AccJerk_Magnitude",colNames[i])
...
tidySet    = aggregate(finalDataNoActivityType[,names(finalDataNoActivityType) != c('activityId','subjectId')],by=list(activityId=finalDataNoActivityType$activityId,subjectId = finalDataNoActivityType$subjectId),mean);

# Merging the tidySet with activityType to include descriptive acitvity names
